...
try:
    ...
    logger.warning(
        "neuroCombat (neuroHarmonize) not available. "
        "Install with: uv add neuroHarmonize"
    )
...
def harmonize_data(
    ...
    """
    Harmonize neuroimaging data across different
    sites/scanners using neuroCombat.
    ...
    logger.info(
        f"Starting harmonization for {data.shape[0]} "
    ...
    if batch_col not in covars.columns:
        # If specified batch_col doesn't exist, try to
        # rename 'site' to the expected column
    ...
    try:
        ...
        harmonized_output = harmonizationLearn(
            data_array.T, covars_encoded, return_s_data=False
        )

        logger.info("Harmonization completed successfully")

        # Return subjects x features
...
def apply_harmonization(
    data: np.ndarray | pd.DataFrame, covars: pd.DataFrame, model: dict
) -> np.ndarray:
    """
...
def create_example_data(
    n_subjects: int = 100, n_features: int = 50, n_sites: int = 3
    ...
    covars = pd.DataFrame({"SITE": sites, "age": ages, "sex": sex})

    logger.info(
        f"Created example data: {n_subjects} subjects, "
        f"{n_features} features, {n_sites} sites"
    )
