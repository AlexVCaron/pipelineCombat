...
class NeuroStatAnalyzer:
    ...
    def multiple_comparisons_correction(
        self, pvalues: np.ndarray, method: str = "fdr_bh", alpha: float = 0.05
    ...
    def correlation_analysis(
        self,
        data: np.ndarray | pd.DataFrame,
        behavioral_scores: np.ndarray | pd.Series,
        method: str = "pearson",
        feature_names: list[str] | None = None,
    ) -> dict[str, Any]:
        """
        ...
        for i, feature_name in enumerate(feature_names):
            ...
            try:
                corr, p_val = corr_func(
                    feature_data[valid_mask], behavioral_scores[valid_mask]
                )
...
def create_example_behavioral_data(
    n_subjects: int = 100,
    ...
    covariates = pd.DataFrame({"age": ages, "sex": sex, "group": groups})

    logger.info(
        f"Created example data: {n_subjects} subjects, "
        f"{n_regions} brain regions"
    )
